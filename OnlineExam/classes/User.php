...
class User{
	...
	public function userRegistration($name,$username,$password,$email){
		...
		if(($name == "" )|| ($username == "") || ($password== "") || ($email== "")){
			echo "<span class='error'>Remplir obligatoirement tous les champs!!</span>";
			exit();
		}else if(filter_var($email,FILTER_VALIDATE_EMAIL) === false){
			echo "<span class='error'>Address Email Invalide  </span>";
			exit();
			...
			if($chkresult != false){
				echo "<span class='error'>L'adresse email existe déjà!!</span>";
				exit();
			}else{
				$password = mysqli_real_escape_string($this->db->link, md5($password));
				...
				if($inserted_row){
				    echo "<span class='success'>L'enregistrement est un succes</span>" .
				    "<span>votre nom est:$name</span>" ;
					exit();
				}else{
					echo "<span class='error'>Erreur..L'enregistrement a échoué !!!</span>";
					exit();
				}
			}
		}
